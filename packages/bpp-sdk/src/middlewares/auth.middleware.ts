...
const createAuthorizationHeader = async (message: any, sdk: BppSDK) => {
  const { signingString, expires, created } = await createSigningString(JSON.stringify(message))
  const signature = await signMessage(signingString, sdk.options.app?.private_key || '')
  const subscriberId = sdk.options.app?.id || ''
  const header = `Signature keyId="${subscriberId}|${sdk.options.app?.unique_id}|ed25519",algorithm="ed25519",created="${created}",expires="${expires}",headers="(created) (expires) digest",signature="${signature}"`
  return header
}
